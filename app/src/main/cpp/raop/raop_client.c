...
typedef struct raopcl_s {
	...
	char rsaaeskey_b64[512]; // RSA-OAEP(key), base64 ('=' stripped) — computed in Kotlin
...
/*----------------------------------------------------------------------------*/
// Fill a buffer with random bytes from /dev/urandom (replaces OpenSSL RAND_bytes).
// The RSA-OAEP encryption of the AES key is done in Kotlin (java.security) and the
// resulting base64 key is supplied via raopcl_set_aes(), so no RSA is needed here.
static void rnd_bytes(uint8_t *buf, size_t len)
{
...
/*----------------------------------------------------------------------------*/
// Supply the AES-128-CBC key/iv plus the precomputed RSA-OAEP(key) base64 string
// (the RSA step is done in Kotlin). Call after raopcl_create() and before raopcl_connect().
bool raopcl_set_aes(struct raopcl_s *p, uint8_t *key, uint8_t *iv, const char *rsaaeskey_b64)
{
	if (!p) return false;
	memcpy(p->key, key, 16);
	memcpy(p->iv, iv, 16);
	strncpy(p->rsaaeskey_b64, rsaaeskey_b64, sizeof(p->rsaaeskey_b64) - 1);
	p->rsaaeskey_b64[sizeof(p->rsaaeskey_b64) - 1] = '\0';
	aes_set_key(&p->ctx, p->key, 128);
	return true;
}
							   ...
							   int sample_rate, int sample_size, int channels, float volume)
{
	...
	rnd_bytes(raopcld->iv, sizeof(raopcld->iv));
	rnd_bytes(raopcld->key, sizeof(raopcld->key));
	// NB: key/iv (and the matching base64 rsaaeskey) are normally overwritten by
	// raopcl_set_aes() before connect; this is just a sane fallback.
   ...
   // codec
	...
	switch (p->crypto ) {
		case RAOP_RSA: {
			...
			// RSA-OAEP(key) was computed in Kotlin and handed to us via raopcl_set_aes().
			key = strdup(p->rsaaeskey_b64);
...
bool raopcl_connect(struct raopcl_s *p, struct in_addr peer, uint16_t destport, bool set_volume)
{
	...
	rnd_bytes((uint8_t*) &p->ssrc, sizeof(p->ssrc));

	p->encrypt = (p->crypto != RAOP_CLEAR);
	memset(&p->sane, 0, sizeof(p->sane));
	p->retransmit = 0;

	rnd_bytes((uint8_t*) &seed, sizeof(seed));
