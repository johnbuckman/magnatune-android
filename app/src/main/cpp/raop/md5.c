/*
 * Compact MD5 implementation, public domain (derived from the RFC 1321 reference /
 * Alexander Peslyak's public-domain implementation). Exposes an OpenSSL-style one-shot
 * MD5(data, len, out). Used only for optional RTSP digest auth.
 */
#include "md5.h"
#include <string.h>

typedef struct { uint32_t lo, hi; uint32_t a, b, c, d; unsigned char buffer[64]; } MD5_CTX;
...
static const void *body(MD5_CTX *ctx, const void *data, unsigned long size) {
	...
	uint32_t a, b, c, d, saved_a, saved_b, saved_c, saved_d, block[16];
