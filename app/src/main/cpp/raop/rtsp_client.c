...
bool rtspcl_pair_verify(struct rtspcl_s *p, char *secret_hex) {
	// AirPlay 2 pairing (ed25519/X25519) - not used for AirPlay 1 (RSA) devices.
	(void) secret_hex;
	LOG_ERROR("[%p]: pair_verify not supported (AirPlay 1 build)", p);
	return false;
}

/*----------------------------------------------------------------------------*/
// FairPlay/MFiSAP auth-setup (et=4): POST an X25519 public key to /auth-setup. Required by
// AirPlay-2 receivers (Sonos, macOS, AppleTV) before they accept a stream, even unencrypted.
// Uses the bundled pure-C curve25519 (no OpenSSL). pair_verify (PIN pairing) stays stubbed.
bool rtspcl_auth_setup(struct rtspcl_s *p) {
	if (!p) return false;

	uint8_t secret[SECRET_KEY_SIZE];
	uint8_t *pub_key = malloc(PUBLIC_KEY_SIZE + 1);
	uint8_t *rsp = NULL;
	int rsp_len = 0;

	// random X25519 private scalar from /dev/urandom
	int fd = open("/dev/urandom", O_RDONLY);
	if (fd >= 0) { (void)! read(fd, secret, SECRET_KEY_SIZE); close(fd); }

	curve25519_dh_CalculatePublicKey(pub_key + 1, secret);
	pub_key[0] = '\x01';

	bool ok = exec_request(p, "POST", "application/octet-stream", (char *) pub_key,
						   PUBLIC_KEY_SIZE + 1, 1, NULL, NULL, (char **) &rsp, &rsp_len, "/auth-setup");
	free(pub_key);
	free(rsp);
	if (!ok) LOG_ERROR("[%p]: auth-setup failed", p);
	return ok;
}
